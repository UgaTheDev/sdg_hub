...
Use `output_columns` when you want SDG Hub to prune intermediate columns and keep only selected generated columns (while preserving original input columns). Runtime pruning is conservative: SDG Hub only drops columns early when a last consumer is known from declared block `input_cols`, and defers undeclared-consumer columns to final cleanup. If you set this field, provide at least one generated column and ensure each listed column is produced by the flow, otherwise execution fails with a validation error.
