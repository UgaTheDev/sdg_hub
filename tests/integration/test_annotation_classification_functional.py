...
def test_annotation_flows_functional(temp_output_dir: Path):
    ...
    # Create mock setup cell to inject at the beginning
    mock_setup_cell = {
        "cell_type": "code",
        "execution_count": None,
        "metadata": {"tags": ["injected-mock"]},
        "outputs": [],
        "source": [
            "# Mock setup for annotation flows testing\n",
            "from unittest.mock import MagicMock, patch\n",
            "\n",
            "# Mock categories that match guided_choice in flow configs\n",
            "expected_categories = ['World', 'Sports', 'Business', 'Sci/Tech']\n",
            ...
            "openai_patcher.start()\n",
            "\n",
            "print('✅ Mock LLM setup complete for annotation flows testing')"
        ]
    }
...
def test_annotation_data_pipeline_integrity(temp_output_dir: Path):
    ...
    mock_setup_cell = {
        ...
        "outputs": [],
        "source": [
            "# Mock setup for annotation data pipeline testing\n",
            "from unittest.mock import MagicMock, patch\n",
            "\n",
            "# Mock categories that match guided_choice in flow configs\n",
            "expected_categories = ['World', 'Sports', 'Business', 'Sci/Tech']\n",
            "mock_responses = []\n",
            "for i in range(200):  # Enough for full notebook\n",
            "    category = expected_categories[i % len(expected_categories)]\n",
            "    mock_response = MagicMock()\n",
            "    mock_response.choices = [MagicMock()]\n",
            "    mock_response.choices[0].message.content = category\n",
            ...
            "openai_patcher.start()\n",
            "\n",
            "print('✅ Mock LLM setup complete for annotation data pipeline testing')"
        ]
    }
